...
// MongoDB connection
mongoose.connect("mongodb://127.0.0.1:27017/userSide")
  .then(() => console.log("MongoDB connected"))
  .catch((err) => console.error("MongoDB connection error:", err));

// Route to store student information (POST)
app.post("/studentInfo", (req, res) => {
    studentInfoModel.create(req.body)
...
// Route to fetch student information (GET)
app.get("/student-form-edit", (req, res) => {
    studentInfoModel.find()
...
// Route to delete student information (DELETE)
app.delete("/student-form-edit/:id", (req, res) => {
    const { id } = req.params;
    studentInfoModel.findByIdAndDelete(id)
        .then(result => {
            if (result) {
...
// Route to fetch room details (GET)
app.get("/room-details", (req, res) => {
    roomDetailsModel.find()
...
// Route to fetch a single room detail by room_no (GET)
app.get("/room-details/:room_no", (req, res) => {
    const { room_no } = req.params;
    roomDetailsModel.findOne({ room_no })
    .then(roomDetail => {
      if (roomDetail) {
        res.json(roomDetail);
      } else {
...
// Route to update room details (PUT)
...
// Route to fetch rules details (GET)
app.get("/rules-details", (req, res) => {
    rulesDetailsModel.find()
...
// Route to store new rules (POST)
app.post("/rules-details", (req, res) => {
    rulesDetailsModel.create(req.body)
...
// Route to delete rules (DELETE)
app.delete("/rules-details/:id", (req, res) => {
    const { id } = req.params;
    rulesDetailsModel.findByIdAndDelete(id)
        .then(result => {
            if (result) {
...
// Route to fetch lunch details (GET)
app.get("/lunch-details", (req, res) => {
    lunchDetailsModel.find()
